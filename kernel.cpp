...
(note: might need to run through ssh in CSIF computers for accurate distance floating-point computation)
...
void readInput(string matrix, char* b, string seq1, string seq2){
    ...
    khat = computeKhat(bl62,indexMap, seq1, seq2);//computes khati

    clock_t end = clock();

    cout<< "distance: " << khat << endl;//test print
...
double computeK(double* bl62[], short* indexMap, string seq1, string seq2){
    ...
    for(int k = 1; k<= s_max; k++){
	 total_K += sumK(bl62, indexMap, seq1, seq2, k); 
